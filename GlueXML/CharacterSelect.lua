...
function UpdateCharacterList()
	...
	for i=1, numChars, 1 do
		...
		local name, race, class, levelQ, zone, sex, ghost, PCC, PRC, PFC = GetCharacterInfo(i);
		local twenties = 1
		local level = 0
		while (levelQ > 0) do
			local reduceQP = min(levelQ, (20 * twenties));
			levelQ = levelQ - reduceQP;
			level = level + floor(reduceQP / twenties)
			twenties = twenties + 1;
		end
