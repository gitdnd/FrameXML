...
local NeoTalentFrame = 0
function NeoTalentUI.OnLogin() 
	SendClientRequest(config.Prefix, 1)
end

function OnCacheReceived(sender, argTable)
	NeoTalentUI.cache = argTable[1]  
end
...
EventFrame:SetScript("OnEvent", function() NeoTalentUI.OnLogin() end)
 
local inte = 0

function OnTalentDevelopmentReceived(sender, argTable)  
	local dot1, dot2 = string.find( argTable[1], "%." )  
	local v = string.sub( argTable[1], 1, dot1 - 1 )
	local k = string.sub( argTable[1], dot1 + 1 ) 
	NeoTalent["Development"][k] = v
	if(NeoTalentFrame ~= 0) then
		TalentFrame_Update(NeoTalentFrame) 
	end 
end

function TalentFrame_Load(TalentFrame) 
	NeoTalentFrame = TalentFrame
	...
	for tab = 1, 3 do
		local numTalents = GetNumTalents(tab, TalentFrame.inspect, TalentFrame.pet);
		for i=1, MAX_NUM_TALENTS do
			if ( i <= numTalents ) then
				...
				local str = GetTalentLink(tab, button:GetID()) 
				str = string.sub(str, 20, string.len(str)) 
				local loc, loc2 = string.find( str, ":" )   
				str = string.sub(str, 1, loc - 1)
				SendClientRequest(config.Prefix, 3, str) 
			end
		end 
	end
end
...
function TalentFrame_Update(TalentFrame) 
	...
	local talentFrameName = TalentFrame:GetName();
	selectedTab = PanelTemplates_GetSelectedTab(TalentFrame);
	...
	for i=1, MAX_NUM_TALENTS do
		...
		if ( i <= numTalents ) then
			...
			if ( name ) then
				...
				local str = GetTalentLink(selectedTab, button:GetID())
				str = string.sub(str, 20, string.len(str)) 
				local loc, loc2 = string.find( str, ":" )   
				str = string.sub(str, 1, loc - 1) 
				_G[buttonName.."Rank"]:SetText( NeoTalent["Development"][str] ); 
...
function SetTalentButtonLocation(button, tier, column)
	...
	button:SetScript("OnClick", function() 
		...
		local loc, loc2 = string.find( str, ":" )   
		str = string.sub(str, 1, loc - 1)
		SendClientRequest(config.Prefix, 2, str)   
		SendClientRequest(config.Prefix, 3, str) 
	end)
end
